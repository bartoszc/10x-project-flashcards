...
- Rejestracja odbywa się na dedykowanej stronie `/auth/register`.
- Formularz rejestracyjny zawiera pola: adres e-mail, hasło oraz potwierdzenie hasła.
- Hasło musi spełniać minimalne wymagania bezpieczeństwa (np. min. 8 znaków).
- Po poprawnym wypełnieniu formularza i weryfikacji danych konto jest aktywowane.
- Użytkownik otrzymuje potwierdzenie pomyślnej rejestracji i zostaje automatycznie zalogowany.
- W przypadku błędów walidacji (np. niepoprawny e-mail, za krótkie hasło, niezgodne hasła) wyświetlane są stosowne komunikaty.
- Nie korzystamy z zewnętrznych serwisów logowania (np. Google, GitHub).
- Użytkownik może przejść do strony logowania, jeśli ma już konto.
...
- Logowanie odbywa się na dedykowanej stronie `/auth/login`.
...
- Błędne dane logowania wyświetlają komunikat o nieprawidłowych danych (bez ujawniania, które pole jest błędne).
- Dane dotyczące logowania przechowywane są w bezpieczny sposób.
- Użytkownik może przejść do strony rejestracji, jeśli nie ma jeszcze konta.
- Użytkownik może przejść do strony odzyskiwania hasła (US-011).
- Przycisk logowania jest dostępny w prawym górnym rogu głównego layoutu dla niezalogowanych użytkowników.
...
Opis: Jako zalogowany użytkownik chcę móc się wylogować z systemu, aby zabezpieczyć dostęp do mojego konta na współdzielonym urządzeniu.
Kryteria akceptacji:

- Przycisk wylogowania jest dostępny w prawym górnym rogu głównego layoutu dla zalogowanych użytkowników.
- Po kliknięciu przycisku wylogowania sesja użytkownika jest kończona.
- Użytkownik zostaje przekierowany na stronę główną lub stronę logowania.
- Po wylogowaniu użytkownik nie ma dostępu do chronionych zasobów bez ponownego zalogowania.
...
- Na stronie logowania znajduje się link "Zapomniałem hasła" prowadzący do strony odzyskiwania hasła `/auth/reset-password`.
- Formularz odzyskiwania hasła wymaga podania adresu e-mail powiązanego z kontem.
- Po wprowadzeniu poprawnego adresu e-mail system wysyła link do resetu hasła.
- Ze względów bezpieczeństwa komunikat o wysłaniu e-maila jest wyświetlany niezależnie od tego, czy konto z podanym adresem istnieje.
- Link do resetu hasła jest ważny przez ograniczony czas (np. 1 godzina).
- Po kliknięciu w link użytkownik trafia na stronę umożliwiającą ustawienie nowego hasła.
- Nowe hasło musi spełniać te same wymagania bezpieczeństwa co przy rejestracji.
- Po pomyślnym zresetowaniu hasła użytkownik może zalogować się przy użyciu nowego hasła.

ID: US-012
Tytuł: Usunięcie konta
Opis: Jako zalogowany użytkownik chcę móc usunąć swoje konto wraz ze wszystkimi danymi, aby skorzystać z prawa do bycia zapomnianym (RODO).
Kryteria akceptacji:

- Opcja usunięcia konta jest dostępna w ustawieniach profilu użytkownika.
- Przed usunięciem konta użytkownik musi potwierdzić swoją decyzję (np. poprzez wpisanie hasła lub frazy potwierdzającej).
- Po potwierdzeniu konto wraz ze wszystkimi powiązanymi danymi (fiszki, sesje generowania, statystyki) zostaje trwale usunięte.
- Użytkownik otrzymuje informację o skutecznym usunięciu konta.
- Po usunięciu konta użytkownik zostaje wylogowany i przekierowany na stronę główną.
- Operacja usunięcia konta jest nieodwracalna.
